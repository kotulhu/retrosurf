...
final class SiteRegistry {
    ...
    /// Unregisters game-created static pages (published via `.registerStaticSite`).
    func removeUserCreatedStaticSites() {
        for (id, site) in sites where site is StaticSite {
            sites.removeValue(forKey: id)
        }
    }
}
